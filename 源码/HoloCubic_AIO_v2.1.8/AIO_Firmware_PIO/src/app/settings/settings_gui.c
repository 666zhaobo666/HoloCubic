...
void display_settings_init(void)
{
    ...
    lv_obj_align(cur_ver_label, LV_ALIGN_TOP_MID, 0, 75);
    ...
    lv_obj_align(qq_label, LV_ALIGN_TOP_MID, 0, 130);
    ...
    lv_obj_align(new_ver_label, LV_ALIGN_BOTTOM_MID, 0, -40);
    lv_label_set_text_fmt(new_ver_label, "#ff0000 love you!");
...
void display_settings(const char *cur_ver, const char *new_ver, lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(title_label, "#00ff00 YY\tCubic#");

    lv_label_set_text_fmt(cur_ver_label, "for YY #ff0000");

    lv_label_set_text(qq_label, "Designed by Simon");

    lv_label_set_text(author_label, "@Simon");
}
