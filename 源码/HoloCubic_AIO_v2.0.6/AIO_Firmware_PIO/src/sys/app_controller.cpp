...
bool AppController::wifi_event(APP_MESSAGE_TYPE type)
{
    ...
    case APP_MESSAGE_WIFI_CONN:
    {
        ...
        Serial.println("mission is start try to connect WiFi");
        Serial.println(sta_wifi_status);
        if (false == sta_wifi_status)
        {
            Serial.println("start try to connect WiFi");
            g_network.start_conn_wifi(sys_cfg.ssid_0.c_str(), sys_cfg.password_0.c_str());
            sta_wifi_status = true;
        }
        m_preWifiReqMillis = millis();
        if ((WiFi.getMode() & WIFI_MODE_STA) == WIFI_MODE_STA && CONN_SUCC != g_network.end_conn_wifi())
        {
            // 在STA模式下 并且还没连接上wifi
            Serial.println("connect WiFi False , later will try again");
            sta_wifi_status = false;    //还没有测试没输入wifi的时候有没有bug
            return false;
        }
    }
    break;
