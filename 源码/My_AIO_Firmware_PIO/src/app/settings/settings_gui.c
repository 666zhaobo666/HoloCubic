...
void display_settings_init(void)
{
    ...
    lv_obj_align(cur_ver_label, LV_ALIGN_BOTTOM_LEFT, 5, -120);
...
void display_settings(const char *cur_ver, const char *new_ver, lv_scr_load_anim_t anim_type)
{
    ...
    if (strcmp(cur_ver, new_ver) != 0)
    {
        lv_label_set_text_fmt(new_ver_label, "Please update your Cubic to #ff0000 v %s#", new_ver);
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
    else
    {
        lv_label_set_text_fmt(new_ver_label, "Cubic #ff0000 v %s# is the latest version.", new_ver);
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
    lv_label_set_text(author_label, "Designed by #00ff00 Joey#");
}
